<template>
  <v-container id="grid" fluid >
    <div id="forms">
      <div id="control-header">
        ...
        <select name="" id="" v-model="mainData.curPriceList">
          ...
          <option v-for="(el, k12) in Object.keys(mainData.priceLists)" :key="el.k12">
            {{mainData.priceLists[k12]}}
          </option>
        </select>
        <v-btn @click.prevent="getDataFromFB">GETDATA</v-btn>
        <v-btn @click.prevent="setDataToFB">SETDATA</v-btn>
        <v-btn small flat @click.prevent="">Save</v-btn></p>
      </div>
      <hr>
      <br>
      <div class="xml-fields-wrapper">
        ...
        <header-shop-descr :xmlValues="xmlValues" :addOption="addOption" :deleteOption="deleteOption"></header-shop-descr>
        <br>
        <hr>
        ...
        <div class="offer">
          ...
          <div>Offer ID:
            ...
            <input disabled v-if="!locData.IdEditable" v-model="xmlValues.Offers[xmlValues.config.curOfferKey].Id">
            <input v-else-if="locData.IdEditable" @blur="verifyID(xmlValues.config.tempOfferId)" maxlength="5" :placeholder="xmlValues.Offers[xmlValues.config.curOfferKey].Id" id="offer-id" type="text" v-model="xmlValues.config.tempOfferId">
            Edit: <input type="checkbox" v-model="locData.IdEditable">
          </div>
          <p>Name: <input type="text" v-model="xmlValues.Offers[xmlValues.config.curOfferKey].data.Name"></p>
          <p>Price: <input type="number" v-model="xmlValues.Offers[xmlValues.config.curOfferKey].data.Price"></p>
          <p>Stock quantity: <input class="input-id" type="text" v-model="xmlValues.Offers[xmlValues.config.curOfferKey].data.StockQ"> Available: <input type="checkbox" v-model="xmlValues.Offers[xmlValues.config.curOfferKey].Available"> <!-- available trigger stock to 0 -->
          </p>

          <p>Pictures: <button @click="addOption(xmlValues.Offers[xmlValues.config.curOfferKey].data.Pics)">+1</button></p>
          <p v-for="(el2, k2) in Object.keys(xmlValues.Offers[xmlValues.config.curOfferKey].data.Pics)" :key="el2.k2">
            <input class="pic-input" type="text" v-model="xmlValues.Offers[xmlValues.config.curOfferKey].data.Pics[k2]">
            <button class="x-btn" @click="deleteOption(xmlValues.Offers[xmlValues.config.curOfferKey].data.Pics, k2)">X</button>
          </p>
          <p>Category ID:
            <select class="input-id" v-model="xmlValues.Offers[xmlValues.config.curOfferKey].data.CatId">
          ...
          <p>Parameters: <button @click="addOption($store.state.xmlValues.Offers[xmlValues.config.curOfferKey].data.Pars, 0, 'Parameter')">+1</button> </p>
          <div v-for="(el3, k3) in Object.keys(xmlValues.Offers[xmlValues.config.curOfferKey].data.Pars)" :key="el3.k3">
            <p>Name: <input type="text" v-model="xmlValues.Offers[xmlValues.config.curOfferKey].data.Pars[k3].Name">
              Description: <input style="width: 15rem" type="text" v-model="xmlValues.Offers[xmlValues.config.curOfferKey].data.Pars[k3].Descr">
              <button class="x-btn" @click="deleteOption($store.state.xmlValues.Offers[xmlValues.config.curOfferKey].data.Pars, k3)">X</button></p>
          </div>

          <p>Vendor: <input type="text" v-model="xmlValues.Offers[xmlValues.config.curOfferKey].data.Vendor"> Url: <input type="text" v-model="xmlValues.Offers[xmlValues.config.curOfferKey].data.Url">
            Currency ID:
            <select class="input-id" v-model="xmlValues.Offers[xmlValues.config.curOfferKey].data.CurId">
              <option disabled value="">Please select one</option>
        ...
        <p>{{Object.keys(xmlValues.Offers)}}</p>
    ...
    <div id="preview">
      <p>Preview XML <input type="checkbox" v-model="locData.showXml"></p>
...
/*     ...mapMutations([
  ...
  methods: {
    ...
    constrOffer(curO) {
      ...
      this.$store.dispatch('addNewOfferAction', {key: key, tempOffer: tempOffer})
    },
/*     addNewOffer (offerIndex, commit ) {
    ...
    getDateAndTime() {
      ...
      this.xmlValues.YmlDate = currentdate.getFullYear() + "-" + (currentdate.getMonth() + 1) + "-" + currentdate.getDate() + " " + currentdate.getHours() + ":" + currentdate.getMinutes();
    },
    ...
    verifyID(newId) {
      ...
      if (newId.length === 5 && parseInt(newId)) {
        if (newId === this.xmlValues.Offers[this.xmlValues.config.curOfferKey].Id) {
    ...
    deleteOffer(curI) {
      if (Object.keys(this.xmlValues.Offers).length > 1) {
        let answer = confirm(`Sure to delete offer ${this.xmlValues.Offers[curI].data.Name} whith ID: ${this.xmlValues.Offers[curI].Id}`);
        if (answer) {
          if ((Object.keys(this.xmlValues.Offers)).length === (parseInt(curI) + 1)) {
